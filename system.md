...
- **Coordinator View:** Team member directorys, registration reviews, event day attendance directorys.
- **Class Representative View:** Scoped classroom directory, verified student counts, targeted announcements, attendance check-in.
...
- **Institutional Chapter Invite Codes (`chapter_invite_codes`):**
  ...
  - Tracks total redemptions and records a real-time directory of onboarded students.
...
- **Verification Methods:**
  ...
  - **Manual Lookup & Bulk Upload:** Fallback verification for offline or paper-based directorys.
...
- **Cluster Structure:** Cluster Lead, Faculty Advisor, Member directory, and weekly learning roadmap milestones.
- **Access Modes:**
...
- **Project Metadata:** Multi-student team directorys, assigned mentors, Git repository URL, live demo URL, progress meter (0–100%), and competition awards.
...
- **Automated Cluster Sync:** Cluster member counts and directorys stay synchronized in real time as members join or leave Discord.
...
| `/cluster` | Chapter | Anyone (@everyone) | Displays the chapter's verified active member count and directory directory. |
...
- **Elevates OS:** Architecture treats leadership as a formal, recurring cycle. Academic terms (e.g. *2026 Executive Council*) record start/end dates, full historical directorys, and mandatory handover notes. Incoming executives are selected through an in-platform **Leadership Application Pipeline** (`applied` → `screening` → `interview` → `selected`).
...
- **Other Systems:** Discord communities operate completely detached from student registries, attendance directorys, and official chapter approvals.
