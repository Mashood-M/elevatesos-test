...
export function Dialog({
  ...
  return (
    <div className="fixed inset-0 z-[var(--z-modal)] flex items-end justify-center p-2.5 sm:p-4 sm:items-center">
      <button
        type="button"
      ...
      <div
        ...
        className={cn(
          ...
          !hasCustomMaxH && "max-h-[min(92dvh,820px)]",
      ...
      >
        {/* Pinned Header */}
        {title || description ? (
          <div className="flex items-start justify-between gap-3 border-b border-border/70 px-4 py-3.5 shrink-0 sm:px-6 sm:py-4">
            <div className="min-w-0 flex-1">
              {title ? (
                <h2
                  id={titleId}
                  className="font-[family-name:var(--font-display)] text-[1.15rem] sm:text-[1.2rem] font-bold tracking-[-0.03em] text-text"
                >
                  {title}
                </h2>
              ) : null}
          ...
          <button
            ...
            onClick={onClose}
            aria-label="Close"
            className="absolute right-3.5 top-3.5 sm:right-4 sm:top-4 z-20 rounded-full p-1.5 text-text-mute hover:bg-bg-hover hover:text-text transition"
          >
            <X size={18} />
          </button>
        )}
        ...
        {footer ? (
          <div className="border-t border-border/70 bg-bg-panel px-4 py-3 shrink-0 sm:px-6 sm:py-3.5">
            {footer}
          </div>
        ) : null}
      </div>
