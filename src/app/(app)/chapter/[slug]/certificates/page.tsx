...
export default function ChapterCertificatesPage({
  ...
  const canManage =
    isExecutiveRole(session.roleKey) || isHqRole(session.roleKey);

  // Chapter-scoped events only
  const chapterEvents = store.events.filter(
    (e) => chapter && e.chapterId === chapter.id,
  );
  ...
  const [selectedEventId, setSelectedEventId] = useState(chapterEvents[0]?.id ?? "");
  const [achievement, setAchievement] = useState<CertificateRecord["achievement"]>("Participation");
  const [issueDate, setIssueDate] = useState(new Date().toISOString().split("T")[0]);
  const [signatory, setSignatory] = useState("Campus Lead");
  ...
  function handleIssueCertificate() {
    ...
    const event = chapterEvents.find((e) => e.id === selectedEventId);
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        eyebrow="Programs"
        title="Certificates"
        description={`Issue and manage certificates for ${chapter.name} events.`}
        actions={
          canManage ? (
      ...
      <TerminalPanel
        ...
        meta={`${filteredCerts.length} verified records · ${chapter.name}`}
        accent="cyan"
      >
                ...
                {filteredCerts.length === 0 ? (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-[13px] text-text-dim">
                      No certificates issued yet. Click &ldquo;Issue Certificate&rdquo; to get started.
                    </td>
                  </tr>
                ) : (
            ...
            <div className="relative p-8 rounded-lg bg-gradient-to-b from-[#111827] to-[#0a0f1d] border-4 border-amber-500/40 text-center text-white shadow-2xl overflow-hidden">
              ...
              <div className="grid grid-cols-2 gap-4 mt-8 pt-6 border-t border-white/10 text-left">
                ...
                <div className="text-right">
                  ...
                  <p className="text-[10px] font-mono text-gray-400">Elevates Campus Lead</p>
                </div>
              </div>
            </div>
